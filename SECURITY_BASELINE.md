...
Note: this fork has no Manhattan WM integration at all — the only secrets
are `CLOUDINARY_API_KEY` / `CLOUDINARY_API_SECRET`, and they live only in
`.env`.
...
including `config.json`. Save text files as UTF-8 without a BOM.
