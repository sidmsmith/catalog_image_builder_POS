...
For this project that is `__version__` in `cib/__init__.py`.
...
This project has no deploy — it runs locally.
...
For this project: `.venv\Scripts\python -m unittest discover -s tests`
(and `python -m compileall -q cib tests`).
